...
void vectime(double *A, int NX, int NY, int NZ, int T, int xb, int yb, int zb, int tb) {
	...
	double (* AV) [yb][zb][VECLEN] = (double(*)[yb][zb][VECLEN])alloc_extra_array(sizeof(double) * yb * zb * VECLEN * 4 * myid);
		...
		for (xx = 0; xx < xblocknum; xx++){
			for (yy = 0; yy < yblocknum; yy++){
				for (zz = 0; zz < zblocknum; zz++){
                    ...
                    vec v_x_plus_0, v_x_minus_0;
                    vec v_y_plus_0, v_y_minus_0;
                            ...
                            if (    xb   <= myxb_threshold \
                                ...
                                for (x = xmin + XSLOPE; x < xmax - STRIDE * VECLEN; x++) {
                                    ...
                                    for (y = myybeg - VECLEN + 1; y < myybeg - VECLEN + 1 + myyb; y++) {
                                        ...
                                        v_center_1 = loadv_3d_x_c_blk(y, z);

                                        for (; z <= myzbeg - VECLEN + 1 + myzb - VECLEN; z += VECLEN) {
                                            
                                            vload(in, B[(t)%2][x+STRIDE*4][y+VECLEN][z+VECLEN]);   // the next x iter in vector
                        
                                            v_center_2 = loadv_3d_x_c_blk(y, z + 1);
                                            v_y_minus_1 = ( y == myybeg - VECLEN + 1 ) ? \
                                                            load_v(x, y, z, 0, -1, 0)\
                                                            : loadv_3d_x_c_blk(y - 1, z);
                                            v_y_plus_1  = ( y == myybeg - VECLEN + 1 + myyb - 1 ) ? \
                                                            load_v(x, y, z, 0, 1, 0) \
                                                            : loadv_3d_x_c_blk(y + 1, z); 
                                            v_x_minus_1 = loadv_3d_x_m_blk(y, z + 0);
                                            v_x_plus_1 = loadv_3d_x_p_blk(y, z + 0);
                                            if(y < myybeg - VECLEN + 1 + myyb-1) {
                                                store_x_pp(v_center_3, x, y, z, 1, 0);
                                            }                                                                                                                // (x, y,z)
                                            Compute_1vector(v_center_1, v_center_0, v_center_2, v_x_minus_1, \
                                                            v_x_plus_1, v_y_minus_1, v_y_plus_1);   //1st   store the newest value to the left vec
                                            
                                            if(y < myybeg - VECLEN + 1 + myyb - 1) {
                                                Input_Output_1(out, v_center_0, in);
                                                store_x_pp(v_center_0, x, y, z, 1, 1);
                                            }
                                            else {  //store newest points of boundary
                                                store_v(v_center_0, x, y, z, 0, 0, 0);
                                            }
                                            //-------------------------------------------------------------------------------------------


                                            v_center_3 = loadv_3d_x_c_blk(y, z + 2);                                                                   
                                            v_y_minus_0 = ( y == myybeg - VECLEN + 1 ) ? \
                                                            load_v(x, y, z, 0, -1, 1)\
                                                            : loadv_3d_x_c_blk(y - 1, z + 1);    // (0,-1,+1)
                                            v_y_plus_0  = ( y == myybeg - VECLEN + 1 + myyb - 1 ) ? \
                                                            load_v(x, y, z, 0, 1, 1) \
                                                            : loadv_3d_x_c_blk(y + 1, z + 1);    // (0,+1,+1)
                                            v_x_minus_0 = loadv_3d_x_m_blk(y, z + 1);
                                            v_x_plus_0 = loadv_3d_x_p_blk(y, z + 1);

                                            Compute_1vector(v_center_2, v_center_1, v_center_3, v_x_minus_0, \
                                                            v_x_plus_0, v_y_minus_0, v_y_plus_0);   //2nd   store the newest value to the left vec
                                            ...
                                            else{   //transpose newest points of boundary
                                                store_v(v_center_1, x, y, z, 0, 0, 1);
                                            }
                                            
                                            //-------------------------------------------------------------------------------------------
                        
                                            v_center_0 = loadv_3d_x_c_blk(y, z + 3);
                                            v_y_minus_1 = ( y == myybeg - VECLEN + 1 ) ? \
                                                            load_v(x, y, z, 0, -1, 2) \
                                                            : loadv_3d_x_c_blk(y - 1, z + 2);    // (0,-1,+2)
                                            v_y_plus_1  = ( y == myybeg - VECLEN + 1 + myyb - 1 ) ? \
                                                            load_v(x, y, z, 0, 1, 2)\
                                                            : loadv_3d_x_c_blk(y + 1, z + 2);    // (0,+1,+2)
                                            v_x_minus_1 = loadv_3d_x_m_blk(y, z + 2);
                                            v_x_plus_1 = loadv_3d_x_p_blk(y, z + 2);                  // (+1,0,+2)

                                            Compute_1vector(v_center_3, v_center_2, v_center_0, v_x_minus_1, \
                                                            v_x_plus_1, v_y_minus_1, v_y_plus_1);   //3rd   store the newest value to the left vec
                                            ...
                                            v_center_1 =  (z > myzbeg - VECLEN + 1 + myzb - VECLEN - VECLEN ) ? \
                                                            load_v(x, y, z, 0, 0, 4)\
                                                        :   loadv_3d_x_c_blk(y, z + 4);
                                            v_y_minus_0 = ( y == myybeg - VECLEN + 1 ) ? \
                                                            load_v(x, y, z, 0, -1, 3) \
                                                            : loadv_3d_x_c_blk(y - 1, z + 3);
                                            v_y_plus_0  = ( y == myybeg - VECLEN + 1 + myyb - 1 ) ? \
                                                            load_v(x, y, z, 0, 1, 3)\
                                                            : loadv_3d_x_c_blk(y + 1, z + 3);    // (0,+1,+2)
                                            v_x_minus_0 = loadv_3d_x_m_blk(y, z + 3);
                                            v_x_plus_0 = loadv_3d_x_p_blk(y, z + 3);                 // (+1,0,+3)

                                            Compute_1vector(v_center_0, v_center_3, v_center_1, v_x_minus_0, \
                                                            v_x_plus_0, v_y_minus_0, v_y_plus_0);   //4th   store the newest value to the left vec
                                        ...
                                        if(z >= myzbeg - VECLEN + 1 + myzb && y < myybeg - VECLEN + 1 + myyb - 1){
								        	...
								        	B[(t ) % 2   ][x-1 + STRIDE * 3][y+3][z+2] = tmp[0];
								        	B[(t + 1) % 2][x-1 + STRIDE * 2][y+2][z+1] = tmp[1];
								        	B[(t) % 2    ][x-1 + STRIDE * 1][y+1][z  ] = tmp[2];
								        	B[(t + 1) % 2][x-1             ][y  ][z-1] = tmp[3];
								        }
                                        for ( ; z < myzbeg - VECLEN + 1 + myzb; z++) {
                                            v_y_minus_1 = load_v(x, y, z, 0, -1, 0);
                                            v_y_plus_1 = load_v(x, y, z, 0, 1, 0);
                                            v_x_minus_1 = load_v(x, y, z, -1, 0, 0);
                                            v_x_plus_1 = load_v(x, y, z, 1, 0, 0);
                                            v_center_2 = load_v(x, y, z, 0, 0, 1);
                                            Compute_1vector(v_center_1, v_center_0, v_center_2, v_x_minus_1, \
                                                            v_x_plus_1, v_y_minus_1, v_y_plus_1);
                                            store_v(v_center_0, x, y, z, 0, 0, 0);
